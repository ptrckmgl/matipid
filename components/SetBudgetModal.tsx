...
export const SetBudgetModal: React.FC<SetBudgetModalProps> = ({
  ...
  const { setBudget, getCurrentMonth, budgets, deleteBudget } = useBudgets();
        ...
        <View className="bg-white rounded-t-[32px] p-6 h-[75%]">
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {expenseCategories.map((cat) => (
                  <TouchableOpacity
                    ...
                    className={`px-4 py-3 rounded-2xl border flex-row items-center gap-2 ${
                      selectedCategoryId === cat.id ? "bg-indigo-600 border-indigo-600" : "bg-white border-slate-200"
                    }`}>
                    <CategoryIcon
                      iconName={cat.icon}
                      ...
                      size={16}
                    />
                    <Text className={`font-medium ${selectedCategoryId === cat.id ? "text-white" : "text-slate-600"}`}>
                      {cat.name}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <View className="mb-8">
              ...
              <View
                className={`flex-row items-center bg-slate-50 p-4 rounded-2xl border ${
                  errors.amount ? "border-red-500" : "border-slate-200"
                }`}>
                <Text className="text-2xl font-bold text-slate-800 mr-2">₱</Text>
                <TextInput
                  className={`flex-1 ${
                    errors.amount ? "text-base italic text-slate-800 opacity-70" : "text-2xl font-bold text-slate-800"
                  }`}
            ...
            <TouchableOpacity
              onPress={handleSave}
              className="bg-indigo-600 p-4 rounded-2xl items-center shadow-lg shadow-indigo-200 mb-4">
            ...
            {budgetId && (
              <TouchableOpacity
                ...
                className="bg-red-500 p-4 rounded-2xl items-center flex-row justify-center gap-2 shadow-lg shadow-red-200 mb-8">
                <Trash2 size={20} color="white" />
                <Text className="text-white font-bold text-lg">Delete Budget</Text>
              </TouchableOpacity>
            )}
          </ScrollView>
        </View>
