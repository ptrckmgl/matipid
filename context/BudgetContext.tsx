import { deleteBudgetFromDB, getAllBudgetsFromDB, upsertBudgetToDB } from "@/utils/db";
import { formatValidationErrors, validateBudget } from "@/utils/validation";
...
export const BudgetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadBudgets = async () => {
    try {
      ...
      Alert.alert("Error", "Failed to load budgets. Please restart the app.");
    }
  };
    ...
    return monthBudgets.map((budget) => {
      ...
      const spent = transactions
        .filter((txn) => {
          ...
          return txn.category === category.name && txn.type === "expense" && txnYear === year && txnMonth === monthNum;
        })
  ...
  const setBudget = async (categoryId: string, limitAmount: number, month: string) => {
    try {
      ...
      const validation = validateBudget({ categoryId, limitAmount, month });
      if (!validation.isValid) {
        const errorMessage = formatValidationErrors(validation.errors);
        Alert.alert("Invalid Budget", errorMessage);
